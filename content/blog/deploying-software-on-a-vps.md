...
description: A guide to setting up continuous deployment on a vps
author: quantinium
date: '2026-03-13'
categories:
  - deployment
...
Before starting, let's define some goals
...
Enter file in which to save the key (/home/quantinium/.ssh/id_ed25519): /home/quantinium/.ssh/testing-ed25519 # change the path where you wanna store the ssh-key pair.
Enter passphrase for "/home/quantinium/.ssh/testing-ed25519" (empty for no passphrase): # add a passphrase for more security
Enter same passphrase again:
Your identification has been saved in /home/quantinium/.ssh/testing-ed25519
Your public key has been saved in /home/quantinium/.ssh/testing-ed25519.pub
...
> cat ~/.ssh/testing-ed25519.pub
ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIODM3k6JXmMRjCnrlPzoGYMMwwG2feHvOilYT2In/ruM me@quantinium.dev
```

Create your instance and grab the server public ip address and let's verify the connection.
...
```bash
~$ apt update && apt upgrade -y # will only work on debian or ubuntu based distributions.
```
...
Now that our packages have been updated, before moving forward let's check if the following file exists and _if_ it exists then reboot your server.
...
Now, let's create a non-root user and copy ssh keys to the newly created user using the following commands
...
Let's disable root login. So, open `sshd_config` by
...
Build and tag each container with both commit hash and `latest` and then push to registry. I'm using commit hash as tag as it gives info about which commit was used while building the containers. Makes debugging in future easier.
...
Before automating deployment, let's verify if everything works normally. So let's copy `docker-compose.yml` and `.env` to server and try running it.
...
Now that our containers are running let's configure nginx to point to them. Before configuring nginx let's add a DNS records for your domain to point to your server
...
Hopefully till now everything worked and our site is now fully working, but if we do changes to our code and then push to our codebase, we have to do everything again. From building containers, tagging them, pushing to ghcr and redeploying. So, now we automate this using [actions](https://docs.github.com/en/actions).

## Building Container Image

Let's create a workflow to build our server container. For this create a new file at `.github/workflows/build-server-container.yml`
...
Here we have defined the workflow configuration to deploy to our vps that does the following things
...
3. Check out code i.e. clone it to the runner
...
With the secrets in place, every push to main will automatically build, push, and deploy your containers. That's the CD part of CI/CD done.
